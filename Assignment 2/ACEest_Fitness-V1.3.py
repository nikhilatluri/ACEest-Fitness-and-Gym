...
class FitnessTrackerApp:
    def __init__(self, master):
        ...
        self.style.configure("TNotebook.Tab", font=("Helvetica", 12, "bold"), foreground=COLOR_TEXT, padding=[15, 8],
                             background=COLOR_BACKGROUND)
        self.style.map("TNotebook.Tab", background=[("selected", COLOR_CARD_BG)],
                       foreground=[("selected", COLOR_PRIMARY)])
        self.style.configure("Primary.TButton", font=("Arial", 11, "bold"), background=COLOR_PRIMARY,
                             foreground=COLOR_CARD_BG, padding=10)
        self.style.map("Primary.TButton", background=[('active', '#388E3C')])
        self.style.configure("Secondary.TButton", font=("Arial", 11, "bold"), background=COLOR_SECONDARY,
                             foreground=COLOR_CARD_BG, padding=10)
    ...
    def create_user_info_section(self):
        ...
        # Name
        tk.Label(info_frame, text="Name:", bg=COLOR_CARD_BG).pack(anchor='w');
        self.name_entry = tk.Entry(info_frame)
        self.name_entry.pack(fill="x")
        # Regn-ID
        tk.Label(info_frame, text="Regn-ID:", bg=COLOR_CARD_BG).pack(anchor='w');
        self.regn_entry = tk.Entry(info_frame)
        self.regn_entry.pack(fill="x")
        # Age
        tk.Label(info_frame, text="Age:", bg=COLOR_CARD_BG).pack(anchor='w');
        self.age_entry = tk.Entry(info_frame)
        self.age_entry.pack(fill="x")
        # Gender
        tk.Label(info_frame, text="Gender (M/F):", bg=COLOR_CARD_BG).pack(anchor='w');
        self.gender_entry = tk.Entry(info_frame)
        self.gender_entry.pack(fill="x")
        # Height
        tk.Label(info_frame, text="Height (cm):", bg=COLOR_CARD_BG).pack(anchor='w');
        self.height_entry = tk.Entry(info_frame)
        self.height_entry.pack(fill="x")
        # Weight
        tk.Label(info_frame, text="Weight (kg):", bg=COLOR_CARD_BG).pack(anchor='w');
        self.weight_entry = tk.Entry(info_frame)
    ...
    def create_log_tab(self):
        ...
        tk.Label(log_card, text="Category:", font=("Inter", 12, "bold"), bg=COLOR_CARD_BG, fg=COLOR_TEXT).grid(row=0,
                                                                                                               ...
                                                                                                               padx=10,
                                                                                                               pady=10)
        self.category_menu = ttk.Combobox(log_card, textvariable=self.category_var, values=list(self.workouts.keys()),
                                          state="readonly", width=30, font=("Inter", 11))
    ...
    def update_progress_charts(self):
        ...
        if sum(values) == 0:
            tk.Label(self.chart_container, text="No workout data logged yet.", font=("Inter", 14, "italic"), fg="#888",
                     bg=COLOR_CARD_BG).pack(pady=100);
            return
        ...
        tk.Label(self.progress_tab, text=f"LIFETIME TOTAL: {total_minutes} minutes logged", font=("Inter", 13, "bold"),
                 bg=COLOR_CARD_BG, fg="#DC3545").pack(pady=(10, 5))
    ...
    def export_weekly_report(self):
        ...
        c.drawString(50, height - 100,
                     f"Height: {self.user_info['height']} cm | Weight: {self.user_info['weight']} kg | BMI: {self.user_info['bmi']:.1f} | BMR: {self.user_info['bmr']:.0f} kcal/day")
        # Table of workouts
        y = height - 140
        ...
        for cat, sessions in self.workouts.items():
            for e in sessions:
                table_data.append(
                    [cat, e['exercise'], str(e['duration']), f"{e['calories']:.1f}", e['timestamp'].split()[0]])
        table = Table(table_data, colWidths=[80, 150, 80, 80, 80])
        table.setStyle(TableStyle(
            [("BACKGROUND", (0, 0), (-1, 0), rl_colors.lightblue), ("GRID", (0, 0), (-1, -1), 0.5, rl_colors.black)]))
...
if __name__ == "__main__":
    ...
    export_btn = ttk.Button(root, text="📄 Export Weekly PDF Report", command=app.export_weekly_report,
